package com.bluewave.reservation.common;

/**
 * Created by Developer on 2016-05-12.
 */
public class Const {
    public static final String EXTRA_ID = "extra.id";
    public static final String EXTRA_STORE = "extra.store";
    public static final String EXTRA_PASSWORD = "extra.password";
    public static final String EXTRA_OPPENT_ID = "extra.oppnent.id";
    public static final String EXTRA_INFO = "extra.info";

    public static final String ACTION_END_GAME = "action.end.game";

    public static final String URL_LOGO = "http://hpl.dothome.co.kr/server/assets/data/store/";
    public static final String TAG = "Reservation";
    public static final String POS_STORE = "store";
    public static final String POS_GAME = "game";
    public static final String REGISTRATION_COMPLETE = "registrationComplete";

}
